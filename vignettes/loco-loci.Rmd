...
LOCI measures feature importance by training models with only each individual feature and measuring how much predictive power each feature provides on its own.
...
- **LOCI**: Lower values indicate more important features (better individual performance)

Note that LOCI values represent the performance difference between using only that feature vs. using all features, so they tend to be larger than LOCO values.
