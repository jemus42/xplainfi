...
For feature $j$, LOCO is calculated as:
$$\text{LOCO}_j = \text{Loss}(\text{model without } j) - \text{Loss}(\text{full model})$$
...
For feature $j$, LOCI is calculated as:
$$\text{LOCI}_j = \text{Loss}(\text{featureless model}) - \text{Loss}(\text{model with only } j)$$
