...
The target function is: `y = 10 * sin(π * x1 * x2) + 20 * (x3 - 0.5)² + 10 * x4 + 5 * x5 + ε`
...
ggplot(plot_data, aes(x = importance, y = reorder(feature, importance), fill = method)) +
  ...
  labs(
    ...
    subtitle = paste("Friedman1 task: 5 important + 5 noise features",
                     "\nRFI conditioned on:", paste(conditioning_set, collapse = ", ")),
    x = "Importance Score (Difference)",
    y = "Feature",
    fill = "Method",
    caption = paste("Using", resampling$iters, "-fold cross-validation")
  ) +
  theme_minimal() +
  theme(
...
Let's examine the stability of our importance estimates:

```{r variability-analysis}
# Analyze variability across iterations for PFI
detailed_scores <- pfi$scores
score_summary <- detailed_scores[, .(
  mean_importance = mean(importance),
  sd_importance = sd(importance),
  cv_importance = sd(importance) / abs(mean(importance)),
  n_iterations = .N
), by = feature]
...
ggplot(score_summary, aes(
  ...
  labs(
    title = "PFI Variability: Mean ± SD",
    subtitle = "Error bars show standard deviation across iterations",
...
The correlation between PFI and RFI with empty conditioning set is `r round(correlation_empty, 3)`, confirming that RFI(∅) ≈ PFI as expected.
