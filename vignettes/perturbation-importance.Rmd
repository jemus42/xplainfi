...
Let's first analyze the interaction scenario where $y = 2 \cdot x_1 \cdot x_2 + x_3 + \epsilon$. Note that x1 and x2 have NO main effects.
...
comp_all <- rbind(
  ...
  pfi_int_results[, .(feature, method = "PFI", importance)],
  cfi_int_results[, .(feature, method = "CFI", importance)],
  # Selected RFI conditional results
  rfi_int_x2_results[feature == "x1", .(feature, method = "RFI|x2", importance)],
  rfi_int_x1_results[feature == "x2", .(feature, method = "RFI|x1", importance)],
  rfi_int_x2_results[feature == "x3", .(feature, method = "RFI|x2", importance)],
  # Add some noise features for completeness
  rfi_int_x2_results[feature %in% c("noise1", "noise2"), .(feature, method = "RFI|x2", importance)]
)
...
digraph independent_dag {
  ...
  x2 [fillcolor='lightblue', label='x2'];
  x3 [fillcolor='lightblue', label='x3'];
  noise [fillcolor='lightgray', label='noise'];
  
  # Outcome
  y [fillcolor='greenyellow', label='y'];
  
  # Direct effects only - no interactions or confounding
  x1 -> y [label='1.5', color='blue'];
  x2 -> y [label='1.0', color='blue'];
  x3 -> y [label='0.5', color='blue'];
  
  # No edge from noise to y
  
  # Layout
  {rank=same; x1, x2, x3, noise}
}
")
```
...
# Combine results
comp_ind <- merge(
  pfi_ind_results[, .(feature, pfi = importance)],
  cfi_ind_results[, .(feature, cfi = importance)],
  by = "feature"
) |>
  merge(
    rfi_ind_results[, .(feature, rfi = importance)],
    by = "feature"
  )
...
comp_ind_long[, method := toupper(method)]
...
1. **PFI detects interacting features but cannot distinguish the source**: When x1 and x2 only contribute through their interaction, PFI still assigns them high importance because permuting either feature destroys the crucial interaction term. This shows a key limitation - PFI cannot separate main effects from interaction effects.

2. **CFI provides more nuanced estimates**: By preserving the conditional distribution, CFI provides somewhat lower but still substantial importance scores, reflecting a more sophisticated understanding of the feature dependencies.

3. **RFI answers targeted conditional questions**: RFI's unique strength is revealed when conditioning on specific features. For example, "x1 given x2" shows extremely high importance because x1 becomes crucial when x2 is fixed. This demonstrates RFI's power for targeted inference and feature selection contexts.
...
### Independent Features (Scenario 3)
...
3. **Noise correctly identified**: All methods correctly assign near-zero importance to the noise feature.
