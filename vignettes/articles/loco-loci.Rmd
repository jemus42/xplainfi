...
LOCO can also be computed using the **macro-averaged** approach (`obs_loss = TRUE`), which follows the original paper formulation by aggregating observation-wise differences with the median of absolute differences: $\mathrm{median}(\{|L(y_i, f_{-j}(x_{i,-j}))| - |L(y_i, f(x_i))|\}_{i=1}^n)$.
