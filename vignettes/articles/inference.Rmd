...
Since `xplainfi` also includes knockoffs via the `KnockoffSampler` and the `KnockoffGaussianSampler`, the latter implemented the second order Gaussian knockoffs also used per default in `{cpi}`, we can recreate its results using `CFI` with the corresponding `sampler`.
The important result however is not the `$importance()`, but the observation-wise losses which are the base for the CPI test.
...
With the observation-wise losses, we can conduct the one-sided t-test also performed by `cpi()` by default:
