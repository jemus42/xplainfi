...
dgp_overview <- data.frame(
  ...
  Challenge = c("Feature correlation", "Mediation effects", "Confounding",
                "Interaction effects", "Baseline (no challenges)", "Mixed effects"),
  `PFI Behavior` = c("High (breaks correlation)", "Shows total effects", "Biased upward", "Low (no main effects)", "Accurate", "Mixed"),
  `CFI Behavior` = c("Low (preserves correlation)", "Shows direct effects", "Less biased", "High (captures interactions)", "Accurate", "Mixed"),
  check.names = FALSE
)
...
This DGP creates highly correlated predictors that illustrate the fundamental difference between PFI and CFI when features are redundant.
...
$$Y = 2 \cdot X_1 + 1.5 \cdot X_2 + X_3 + \varepsilon$$
...
# True coefficients: x1=2.0, x2=1.5, x3=1.0, x4=0
```

### Expected Behavior

- **PFI**: Shows high importance for x1 and x2 because permuting either feature breaks the correlation structure, creating unrealistic data combinations that confuse the model
- **CFI**: Shows low importance for x1 and x2 because conditional sampling preserves the correlation, meaning each feature provides little additional information given the other
- **Interpretation**: 
  - PFI measures marginal importance (how much the feature matters on its own)
  - CFI measures conditional importance (how much the feature matters given other features)
  - Both x1 and x2 are individually important but redundant with each other
...
Expected pattern: PFI shows high importance for correlated x1,x2 while CFI shows low importance (they are redundant given each other)
