...
The `MarginalReferenceSampler` is another type of marginal sampler that samples complete rows from reference data. Unlike `PermutationSampler` which shuffles feature values independently, this sampler preserves within-row dependencies by sampling intact observations.

**Key differences from PermutationSampler:**

- **PermutationSampler**: Shuffles each feature independently → breaks ALL dependencies
- **MarginalReferenceSampler**: Samples complete reference rows → preserves WITHIN-row dependencies
...
marginal_ref = MarginalReferenceSampler$new(task, n_samples = 30L)

# Sample a feature - each row gets values from a randomly sampled reference row
original = task$data(rows = 1:5)
...
To demonstrate the key difference, consider features that are correlated:

```{r correlation-preservation}
# Create task with correlated features
set.seed(123)
n = 50
x1 = rnorm(n)
corr_data = data.table(
	x1 = x1,
	x2 = x1 * 2 + rnorm(n, sd = 0.1), # Highly correlated with x1
	x3 = rnorm(n),
	y = factor(sample(c("A", "B"), n, replace = TRUE))
)
task_corr = as_task_classif(corr_data, target = "y")

# Sample with PermutationSampler (breaks correlations)
perm = PermutationSampler$new(task_corr)
sampled_perm = perm$sample(c("x1", "x2"), row_ids = 1:10)

# Sample with MarginalReferenceSampler (preserves within-row correlations)
ref = MarginalReferenceSampler$new(task_corr, n_samples = 50L)
sampled_ref = ref$sample(c("x1", "x2"), row_ids = 1:10)

# Check correlations
cor_original = cor(task_corr$data()$x1, task_corr$data()$x2)
...
sampled = knn$sample(
	feature = "bill_length",
	row_ids = 1:10,
	conditioning_set = c("bill_depth", "body_mass")
)

original = task$data(rows = 1:10)
data.table(
	bill_depth = original$bill_depth,
	body_mass = original$body_mass,
	original = original$bill_length,
	sampled = sampled$bill_length
)
```
