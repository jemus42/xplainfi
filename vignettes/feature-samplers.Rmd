...
marginal = MarginalSampler$new(task)
marginal$feature_types
```
...
Let's demonstrate both with the marginal sampler:

```{r}
# Sample from stored task (using row_ids)
sampled_task = marginal$sample(
...
sampled_external = marginal$sample_newdata(
  feature = "bill_length_mm",
  newdata = test_data
)
sampled_external
...
The `MarginalSampler` performs simple random permutation of features, breaking their relationship with the target and other features. This is the classic approach used in Permutation Feature Importance (PFI).
...
sampled = marginal$sample("bill_length_mm", row_ids = 1:10)
...
**Use in PFI**: The marginal sampler is used by default in Permutation Feature Importance:
...
# Marginal: breaks all dependencies
marginal_sample = MarginalSampler$new(task_numeric)$sample(
  "x1", row_ids = 1:5
)
...
- **Marginal sampling** produces any value from the marginal distribution
...
|---------|--------------|-------------|-------|----------|
| `MarginalSampler` | All | None | Very fast | PFI, uncorrelated features |
...
1. Use **marginal sampling** when features are independent or for baseline PFI
