...
If it aides interpretation, importances can also be calculates as the *ratio* rather then the *difference* between the baseline and post-permutation losses:
...
Where `iter_rsmp` corresponds to the resampling iteration, i.e., 3 for 3-fold cross-validation, and `iter_repeat` corresponds to the permutation iteration within each resampling iteration, 5 in this case.
While `pfi$importance()` contains the means across all iterations, `pfi$scores()` allows you to manually visualize or aggregate them in any way you see fit.
