...
marginal_imputer = function(learner, x, S, ins, target) {
  ...
  pred_dt[, .(mean_pred = mean(pred)), by = ins_id][, mean_pred]
}
...
sage = function(learner, data, target = "y", loss = mlr3measures::se, batch_count = 5) {
  ...
  for (i in 2:(batch_count + 1)) {
    ...
    # perm is nrow(current_batch_dt) x ncol(data_x) matrix of random integers
    perm = t(replicate(
    ...
    # Initial loss values for each instance
    # Vector of length nrow(current_batch_dt)
    loss_prev = loss(
    ...
    for (col_index in 1:ncol(perm)) {
      ...
      for (row_index in 1:nrow(S)) {
        # For each row (=instance) in the S matrix, selects (via perm) a random feature (column) to include
        # in the coalition (by setting value to TRUE)
